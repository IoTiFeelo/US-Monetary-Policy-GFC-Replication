...
1.  Draw \$ (A,\Sigma)\^{(s)} \sim p(A,\Sigma\| X,Y,\kappa*a\^{(s-1)},*
    \kappa{\Sigma}\^{(s-1)}) \$
...
My model comprises the instrument in the first position, and I will compute impulse response functions assuming that the instrument will affect all the other variables exogenously. The assumptions of @MollerWolf2021LPVARS are satisfied given our Bayesian framework and our cholesky decomposition:
$$
\begin{bmatrix}
...
#| warning: false
  ...
  y                   <- window((y), start=c(1990,2), end=c(2010,12))
  basic.results <- basic.model(y, 12, 5000, c(1990,2), c(2010,12))
...
#| warning: false
  ...
  # plot IRFs extended model
  ############################################################
  load("irf-instrument-fevd2.RData")
  IRF.posterior.inst = IRF.posterior[,1,,] #takes the instruments IRFs
  IRFs.k1           = apply(IRF.posterior.inst,1:2,median) #mean of the S IRFs
  IRFs.inf.k1       = apply(IRF.posterior.inst,1,mean)
  rownames(IRFs.k1) = y.names
  ...
  IRFs.k1.hdi    = apply(IRF.posterior.inst,1:2,hdi, credMass=0.68)
  hh          = 1:(h+1)
  pdf(file="FF-irf-mps2.pdf", height=9, width=12)
